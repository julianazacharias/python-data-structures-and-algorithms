...
class SortedArray:
    ...
    # Reference to the vertex and comparison with the distance to the target
    def insert(self, vertex):
        ...
        for i in range(self.last_position + 1):
            position = i
            if self.values[i].target_distance > vertex.target_distance:
                break
    ...
    def print_search(self):
        if self.last_position == -1:
            ...
            for i in range(self.last_position + 1):
                print(i, ' - ', self.values[i].label, ' - ', self.values[i].target_distance)
...
class Greedy:
  ...
  def search(self, current):
    ...
    if current == self.target:
      ...
      sorted_array = SortedArray(len(current.adjacents))
